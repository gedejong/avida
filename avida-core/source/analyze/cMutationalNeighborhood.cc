...
double cMutationalNeighborhood::ProcessOneStepGenome(cAvidaContext& ctx, cTestCPU* testcpu, cCPUTestInfo& test_info,
                                                     const Genome& mod_genome, sStep& odata, int cur_site)
{
  ...
  if (test_fitness != 0.0) { // Only count tasks if the organism is alive
    const AvidaArray<int>& cur_tasks = test_info.GetColonyOrganism()->GetPhenotype().GetLastTaskCount();
    bool knockout = false;
    bool anytask = false;
                                                   ...
                                                   int cur_site, Genome& mod_genome)
{
  ...
  for (int line_num = cur_site + 1; line_num <= mod_size; line_num++) {
    seq.Insert(line_num, Instruction(0));
                                                   ...
                                                   int cur_site, Genome& mod_genome)
{
  ...
  for (int line_num = cur_site; line_num < mod_size; line_num++) {
    ...
    seq.Insert(line_num, Instruction(cur_inst));
  }
}
...
double cMutationalNeighborhood::ProcessTwoStepGenome(cAvidaContext& ctx, cTestCPU* testcpu, cCPUTestInfo& test_info,
                                                     const Genome& mod_genome, sTwoStep& tdata,
...
void cMutationalNeighborhood::AggregateOneStep(AvidaArray<sStep>& steps, sOneStepAggregate& osa)
{
...
void cMutationalNeighborhood::AggregateTwoStep(AvidaArray<sTwoStep>& steps, sTwoStepAggregate& tsa)
{
  sPendFit* pend = NULL;
