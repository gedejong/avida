...
void cModularityAnalysis::CalcFunctionalModularity(cAvidaContext& ctx)
{
  ...
  const AvidaArray<int> base_tasks = test_info.GetColonyOrganism()->GetPhenotype().GetLastTaskCount();
  ...
  if (base_fitness > 0.0 && does_tasks) {
    ...
    AvidaArray<int> site_num_tasks(max_line);  // number of tasks instruction is used in
    site_num_tasks.SetAll(0);
    AvidaArray<int> sites_per_task(num_tasks); // number of sites involved in each task
    ...
    for (int line_num = 0; line_num < max_line; line_num++) {
      ...
      if (test_info.GetColonyFitness() > 0.0) {
        const AvidaArray<int>& test_tasks = test_info.GetColonyOrganism()->GetPhenotype().GetLastTaskCount();
    ...
    AvidaArray<int> sites_inv_x_tasks(num_tasks + 1);  // # of inst's involved in 0,1,2,3... tasks
    sites_inv_x_tasks.SetAll(0);
    AvidaArray<double> ave_task_position(num_tasks); // mean positions of the tasks in the genome
    ave_task_position.SetAll(0.0);
    AvidaArray<int> task_length(num_tasks);  // distance between first and last inst involved in a task
    ...
    AvidaArray<int> task_position(num_tasks);
    for (int i = 0; i < num_tasks; i++) {
      task_position[i] = 0;
